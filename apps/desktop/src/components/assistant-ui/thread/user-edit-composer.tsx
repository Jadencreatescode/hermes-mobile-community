...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  // This is the one composer that routinely unmounts. Confirming or cancelling
  // an edit must cancel every delayed callback before the DOM disappears and
  // release the active focus target so keyboard routing falls back to chat.
  useEffect(
    () => () => {
  ...
  const replaceTriggerWithChip = useCallback(
    ...
    [aui, closeTrigger, recordUndoPoint, refreshTrigger, rememberInitialDraft, requestEditFocus, scheduleTimeout, trigger]
  )
